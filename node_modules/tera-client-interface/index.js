...
class TeraProcessListener {
    constructor({interval, bypassXigncode, bypassMethod}) {
        if (bypassXigncode)
            console.log("[xigncode-bypass] Enabled")
        ...
        ProcessListener("TERA.exe",
            (process) => {
                try {
                    ...
                    if (bypassXigncode) {
                        try {
                            if (bypassMethod === "classic") {
                                const XigncodeFolder = path.join(path.dirname(process.path), "XIGNCODE");
                                fs.copyFileSync(path.join(__dirname, "xigncode-bypass/x3.xem"), path.join(XigncodeFolder, "x3.xem"));
                                fs.copyFileSync(path.join(__dirname, "xigncode-bypass/xcorona.xem"), path.join(XigncodeFolder, "xcorona.xem"));
                            } else {
                                execFileSync(path.join(__dirname, "injector.exe"), [process.pid, path.join(__dirname, "xigncode-bypass/xigncode-bypass.dll")]);
                            }
...
module.exports = { TeraClientInterfaceServer, TeraProcessListener };
