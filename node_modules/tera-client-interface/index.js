...
class TeraDllInjector {
    constructor({interval, bypassXigncode}) {
        if (bypassXigncode)
            console.log("[proxy] xigncode-bypass Enabled")
        ...
        ProcessListener("TERA.exe",
            (process) => {
                try {
                    ...
                    if (bypassXigncode)
                        execFileSync(path.join(__dirname, "injector.exe"), [process.pid, path.join(__dirname, "xigncode-bypass.dll")]);
                } catch (e) {
...
module.exports = { TeraClientInterfaceServer, TeraDllInjector };
