...
function modifySlsXml(doc, customServers) {
  ...
  // sort order by serverId
  asArray(doc.getElementsByTagName('server'))
    .sort((a, b) => {
      const getServerId = server => asArray(server.childNodes).find(node => node.nodeName === 'id').textContent
      return getServerId(a) - getServerId(b)
    })
    .forEach(server => {
