...
function listModuleInfos(rootFolder) {
    return listModules(rootFolder).map(mod => loadModuleInfo(rootFolder, mod));
}
...
function _loadModuleInfoFile(moduleInfo) {
    ...
    return JSON.parse(fs.readFileSync(moduleInfoFilePath, 'utf8'));
}
...
function _storeModuleInfoFile(moduleInfo, data) {
    ...
    moduleInfoFilePath = path.join(moduleInfo.path, 'module.json');
    fs.writeFileSync(moduleInfoFilePath, JSON.stringify(data, null, 4));
}

function setAutoUpdateEnabled(moduleInfo, enabled) {
    moduleInfoFile = _loadModuleInfoFile(moduleInfo);
